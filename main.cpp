...
void ADD(Node*& root, Node* newNode, Node* tree ) {//adds the integers into the BST
  ...
  if(root == nullptr) {//empty list
    ...
    if(treeD > addD) {//if less than parent go left
      if(tree->getLeft() == nullptr) {//no left child make it the left child
	tree->setLeft(newNode);
      }
      else {//otherwise recursion with left child as parent
	ADD(root, newNode, tree->getLeft());
      }
    }
    else if(treeD <= addD) {//if greater than or equal to parent
      if(tree->getRight() == nullptr) {//no right child make it the right child
	tree->setRight(newNode);
      }
      else {//otherwise recursion with right child as parent
